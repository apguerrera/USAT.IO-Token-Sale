...
const USATToken = artifacts.require('USATToken')
...
const MAX_AMOUNT = '50000000000000000000' //50ether
...
contract('TokenSale', function (accounts) {
  describe('initialization', () => {
    it('should deploy KYCBaseTester', async () => {
      await TokenSale.new([SIGNER_ADDR], USATToken.address, accounts[0], 
    ...
    it('should save signer', async () => {
      const ts = await TokenSale.new([SIGNER_ADDR], USATToken.address, accounts[0], 
        web3.eth.getBlock(web3.eth.blockNumber).timestamp,
        web3.eth.getBlock(web3.eth.blockNumber).timestamp + 3600,
      10000, MAX_AMOUNT)
  ...
  describe('buyTokens()', () => {
    ...
    let usat
    before(async () => {
      usat = USATToken.at(USATToken.address);
      /*
      var event_usat = usat.allEvents([]);
      event_usat.watch(function(error, event){
        if (!error)
          console.log(event);
      });
      */
    ...
    beforeEach(async () => {
      ...
      ts = await TokenSale.new([SIGNER_ADDR], USATToken.address, accounts[0], 
        web3.eth.getBlock(web3.eth.blockNumber).timestamp,
        web3.eth.getBlock(web3.eth.blockNumber).timestamp + 3600,
      10000, MAX_AMOUNT)
      ...
      */
      ...
      await USAT.approve(ts.address, 10000000000000000000000000);
    })
  ...
  describe('buyTokensFor', () => {
    it('should fulfill', async () => {
      ...
      const ts = await TokenSale.new([SIGNER_ADDR], USATToken.address, accounts[0], 
        ...
        web3.eth.getBlock(web3.eth.blockNumber).timestamp + 3600,
      10000, MAX_AMOUNT)
      const USAT = USATToken.at(USATToken.address);;
      await USAT.approve(ts.address, 10000000000000000000000000);
